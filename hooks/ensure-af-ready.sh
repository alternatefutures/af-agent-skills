...
#
# Runs before every Bash tool invocation. Detects `acc …` commands and
...
#
# Non-`acc` commands are passed through (exit 0 immediately).
#
...
# Cheap detection: is the command line invoking `acc` (as a word)?
...
case "$INPUT" in
  *'"command":'*'"acc '*|*'"command":'*' acc '*|*'"command":'*'"af"'*)
    : # falls through to the af-readiness check below
    ;;
  *)
    # Not an `acc` command — let it run.
    exit 0
    ;;
esac
...
if ! command -v acc >/dev/null 2>&1; then
  cat >&2 <<'EOF'
✗ The Alternate Clouds CLI (`acc`) is not installed.
...
Then run `acc login` to sign in. After that, re-run the request.
EOF
  exit 1
fi

# Is the user logged in? `acc whoami --json` exits non-zero when not authed.
# We swallow stderr so the user doesn't see the inner "not logged in" twice.
if ! acc whoami --json >/dev/null 2>&1; then
  cat >&2 <<'EOF'
...
Then re-run the request. (Use `acc login --email` for terminal-only flow.)
EOF
  exit 1
fi
