...
fingerprint) and a shared room + password. Each agent `read`s for tasks/replies
...
- **Confidentiality rests entirely on the room password.** It derives both the
  AES key and the room id, so a wrong password lands you in a *different, empty*
...
- **The fingerprint identifies a peer, not the display name.** Display names are
  carried in the ciphertext and are *not* authenticated — anyone with the room
  password can announce any name. Trust the Ed25519 fingerprint, not the name.
